...
public class TestEtradeRepository {
    ...
    private NordnetRedis getNordnetRedis() {
        return new NordnetRedis("172.20.1.2", 5);
    }
    /*
    ...
    public void testCalls() {
        ...
        repos.setCurrentDate(LocalDate.of(2020,10,5));
        repos.setNordnetRedis(getNordnetRedis());
        Collection<StockOptionPrice> calls = repos.calls(2);
        assertThat(calls.size()).isEqualTo(5);

        String ticker = "EQNR0K125";
        Optional<StockOptionPrice> call = calls.stream().filter(x -> x.getTicker().equals(ticker)).findAny();
        ...
        call.ifPresent( c -> {
            ...
            assertThat(c.optionPriceFor(123.0)).as("Option price for").isGreaterThan(0.0);
        });
    }
    ...
    public void testPuts() {
        ...
        Optional<StockOptionPrice> put = puts.stream().filter(x -> x.getTicker().equals(ticker)).findAny();
